import languageData from '../src/data/languageStructure.json' assert { type: 'json' };

const flattenLanguageParameters = (data) => {
    let result = [];

    const extractParameters = (items) => {
        items.forEach((item) => {
            if (!item.children && item.parameter) {
                result.push(item.parameter);
            }
            if (item.children && item.children.length > 0) {
                extractParameters(item.children);
            }
        });
    };
...
export default {
    ...
    locales: flattenLanguageParameters(languageData),
